...
export function scoreRound(
  ...
  const bidSidePoints = teams.bidderSide.reduce<number>((sum, s) => sum + (capturedPoints[s] ?? 0), 0);
  const otherSidePoints = teams.otherSide.reduce<number>((sum, s) => sum + (capturedPoints[s] ?? 0), 0);
  const madeBid = bidSidePoints >= bidAmount;

  const deltas: Record<Seat, number> = { 0: 0, 1: 0, 2: 0, 3: 0 };

  if (madeBid) {
    for (const s of teams.bidderSide) deltas[s] = bidSidePoints;
    for (const s of teams.otherSide) deltas[s] = otherSidePoints;
  } else {
    for (const s of teams.bidderSide) deltas[s] = -bidAmount;
    for (const s of teams.otherSide) deltas[s] = TOTAL_ROUND_POINTS;
  }
 ...
 */
export function checkForWinners(scores: Record<Seat, number>, winThreshold: number): Seat[] {
