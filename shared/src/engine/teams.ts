import type { Card, RoundTeams, Seat, Suit } from '../models.js';
...
export function determineTeams(
  ...
  let aceHolder: Seat | undefined;
  let twoHolder: Seat | undefined;

  for (const seat of SEATS) {
    for (const card of hands[seat]) {
      if (card.kind === 'standard' && card.suit === trumpSuit) {
        if (card.rank === 'A') aceHolder = seat;
        if (card.rank === '2') twoHolder = seat;
      }
    }
  }
  ...
  const acesSide: Seat[] = soloSide ? [aceHolder] : [aceHolder, twoHolder];
  const rest: Seat[] = SEATS.filter((s) => !acesSide.includes(s));
