import type { Card, Rank, Suit } from '../models.js';
...
export function offSuit(trumpSuit: Suit): Suit {
  switch (trumpSuit) {
    case 'hearts':
      return 'diamonds';
    case 'diamonds':
 ...
 * 7, 6, 5, 4, 3, off3, 2.
 */
export function trumpRankIndex(card: Card, trumpSuit: Suit): number | undefined {
  if (card.kind === 'joker') {
...
export function pointValue(card: Card, trumpSuit: Suit): number {
  const idx = trumpRankIndex(card, trumpSuit);
  if (idx === undefined) return 0;
  if (ONE_POINT_INDICES.has(idx)) return 1;
  if (THREE_POINT_INDICES.has(idx)) return 3;
  return 0;
}
...
export function trumpCardsInHand(hand: Card[], trumpSuit: Suit): Card[] {
  return hand
